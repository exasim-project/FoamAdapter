...
ShapeModel = ShapeBase.plugin_model

def test_valid_configs_n_models():
    m1 = ShapeModel(shape={"shape_type": "circle", "radius": 2.5}, color="red")
    assert m1.shape.radius == 2.5
    m2 = ShapeModel(shape={"shape_type": "square", "side": 4.0}, color="blue")
    assert m2.shape.side == 4.0
    m3 = ShapeModel(shape={"shape_type": "rectangle", "width": 3.0, "height": 6.0}, color="green")
    assert m3.shape.width == 3.0
    assert m3.shape.height == 6.0

def test_invalid_config_n_models():
    with pytest.raises(ValidationError):
        ShapeModel(shape={"shape_type": "circle"}, color="bad")  # missing 'radius'
    with pytest.raises(ValidationError):
        ShapeModel(shape={"shape_type": "square", "radius": 2.0}, color="bad")  # wrong field

def test_extensibility_n_models():
    ShapeBase.register(TriangleConfig)
    m4 = ShapeBase.create(shape={"shape_type": "triangle", "base": 3.0, "height": 4.0}, color="yellow")
    assert m4.shape.base == 3.0
    assert m4.shape.height == 4.0
    assert m4.shape.shape_type == "triangle"

    ShapeBase.register(PolygonConfig)
    m5 = ShapeBase.create(shape={"shape_type": "polygon", "sides": 5, "length": 2.0}, color="purple")
...
def test_json_schema():
    ShapeModel = ShapeBase.plugin_model
    schema = ShapeModel.model_json_schema()
