...
private:
  ...
  void computeFaceAddressing() {
    ...
    int32_t lclIdx = 0;
    for (int32_t itree = 0; itree < t8_forest_get_num_local_trees(forest_);
         itree++) {
      t8_eclass_t tree_class = t8_forest_get_tree_class(forest_, itree);
      t8_eclass_scheme_c *eclass_scheme =
          t8_forest_get_eclass_scheme(forest_, tree_class);
      for (int32_t ielement = 0;
           ielement < t8_forest_get_tree_num_elements(forest_, itree);
           ielement++) {

        const t8_element_t *element =
            t8_forest_get_element_in_tree(forest_, itree, ielement);
        t8_forest_element_centroid(forest_, itree, element,
                                   elem_midpoint.data());
        ...
        for (int iface = 0; iface < num_faces; iface++) {
          ...
          t8_forest_leaf_face_neighbors(forest_, itree, element, &neighbors,
          ...
          if (num_neighbors > 0) {
            ...
            t8_forest_element_face_normal(forest_, itree, element, iface,
                                          face_normal.data());
            t8_forest_element_face_centroid(forest_, itree, element, iface,
                                            face_centroid.data());
            // t8_element_level()
            ...
            std::cout << "face_normal: " << face_normal[0] << " "
                      << face_normal[1] << " " << face_normal[2] << std::endl;
            ...
            auto max_it = std::max_element(
                face_normal.begin(), face_normal.end(),
            ...
            if (face_normal[orient] > 0.0) {
              ...
              switch (orient) {
              case 0:
                Ax_.push_back(t8_forest_element_face_area(forest_, itree,
                                                          element, iface));
                break;
              case 1:
